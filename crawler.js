...
async function main() {
  try {
    ...
    const CC_LIST = await playerSearch([289], 0); // playerSearch(시즌넘버, 최소오버롤)
    let CC_RESULTS = await playerPriceValue(CC_LIST, [1, 2, 3, 4, 5, 6, 7, 8]); // playerPriceValue(데이터 , 강화등급)
    await saveToDB(CC_RESULTS);
